...
module.exports = async (req, res) => {
  ...
  try {
    const r = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        from: 'SpeakUpESL <noreply@speakup-esl.org>',
        to: guardianEmail,
        subject: `Permission needed: ${username || 'your child'} wants to join SpeakUpESL`,
        html: `<p>Hi,</p>
<p><strong>${username || 'Your child'}</strong> is trying to create an account on SpeakUpESL, an English-practice app for ESL learners. Because they're under 13, we need your permission before their account can be used.</p>
<p>SpeakUpESL will store their name, email, age, English level, and practice conversations (which are sent to an AI provider, Anthropic, to generate replies and feedback).</p>
